function [] = imdif_scramble_run_importance_sampler_tacc(scramble_run)
...
analysis_settings.scramble_run = scramble_run; % will need to specify a scramble sample number. This will need to be unique for each sample
...
start_dir = pwd;
  ...
  data_file = fullfile(data_dir, sprintf('imdif_net_pcit_data_windowstart%s_windowlen%s_%s.mat',trstart,trlens,target));
  load(data_file);
  original_data = data;
...
%   analysis_settings.runs = run;
  ...
  sc_data = data;
  imdif_importance_sampler(sc_data, analysis_settings);
